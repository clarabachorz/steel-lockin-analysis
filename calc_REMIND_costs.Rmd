...
calc_cumu_investment_costs <- function(df, discount = 0.05){
  ...
  df_discounted <- df %>%
    ...
    arrange(region, scenario, component, period) %>%
    mutate(discounted_cost = value1 / ((1 + discount) ^ (period - 2025))) %>%
    group_by(region, scenario, component) %>%
...
df.totalcosts2 <- calc_total_costs(base, scenario_name = "polextended", include_co2_price = FALSE, start_year = 2025, end_year = 2070)
...
# Area plot
region_plot <- ggplot() +
  geom_area(data = plot_df,
          aes(x = period, y = cumulative_value, fill = factor(component)),
          position = "stack", 
          stat="identity", 
          width=3) +
  scale_fill_manual(values = component_colors) +
  facet_wrap(~ scenario, nrow = 3) +
  labs(
    title = "Cumulative investment costs for India, for a `NPi`, `Extended lock-in` and `Fast\ntransition` scenario (820 PkBudget)",
    x = "Year",
    y = "Investment costs\n(Bill. USD) per year",
    fill = "Component"
  ) +
...
#save
# ggsave("figs_temp/investment_costs_cumu_820_IND.png", plot = region_plot, width = 8, height = 13, dpi = 300)
```
