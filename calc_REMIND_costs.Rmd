...
df.totalcosts <- bind_rows(df.totalcosts1$cumu, df.totalcosts2$cumu) %>%
  ...
  mutate(diff_scenarios = greenpol - polextended) %>%
  mutate(perc_diff_scenarios = diff_scenarios / (`polextended` + `greenpol`) * 100)

df.totalcosts_aggr <- df.totalcosts %>%
  group_by(region, period) %>%
  summarise(total_greenpol = sum(greenpol),
            total_base = sum(polextended)) %>%
...
global_plot <- ggplot(plot_df_global, aes(x = period, y = diff_scenarios, fill = component)) +
  ...
  facet_wrap(~ region, scales = "free_y") +
  labs(
    title = "Cumulative cost difference between the green policy 650 and NPi scenarios",
    # title = "Cumulative cost difference between GreenPolicy and PolicyExtended\nfor 650PkBudget",
    x = "Year",
    y = "Cumulative cost difference\n(trUSD)",
    fill = "Component"
  ) +
  ...
  geom_text(aes(x = 2029, y = max(plot_df_global$diff_scenarios) * 0.9, label = "GreenPol650 more expensive\nthan NPi"), color = "#797777", size = 4, hjust =0.3) +
  annotate("segment", x = 2025, xend = 2025, y = 0, yend = -max(plot_df_global$diff_scenarios)*0.6, arrow = arrow(length = unit(0.2, "cm")), color = "#797777")+
  geom_text(aes(x = 2029, y = -max(plot_df_global$diff_scenarios) * 0.3, label = "GreenPol650 less expensive\nthan NPi"), color = "#797777", size = 4, hjust =0.3)

final_plot <- gridExtra::grid.arrange(global_plot,region_plot,  ncol = 1)

print(final_plot)
#save
# ggsave("figs_temp/costs_greenpol_vs_polextended_650_noCO2.png", plot = final_plot, width = 10, height = 12, dpi = 300)
# ggsave("figs_temp/costs_greenpol_vs_NPi_1000_reg.png", plot = final_plot, width = 10, height = 12, dpi = 300)
ggsave("figs_temp/costs_greenpol_650_vs_NPi_wCO2.png", plot = final_plot, width = 10, height = 12, dpi = 300)
```

```{R calculate investment costs}

greenpol <- "inputdata/gdx/GreenPolicy-PkBudg650.gdx"
# base <- "inputdata/gdx/PolicyExtended-PkBudg1000.gdx"
npi <- "inputdata/gdx/GreenPolicy-NPi.gdx"
...
```{R tests}
df.totalcosts1 <- df.totalcosts1 %>%
  # bind_rows(df.totalcosts2) %>%
  bind_rows(df.totalcosts3) %>%
  filter(period >= 2030, period <= 2070)
...
# scen_order <- c( "NPi", "Policy extended", "Green policy")
...
region_plot <- ggplot() +
  ...
  labs(
    title = "Investment costs for SSA, for a `NPi` and `Green\npolicy push` scenario (650 PkBudget)",
    x = "Year",
    y = "Investment costs\n(Bill. USD)",
    fill = "Component"
  ) +
...
#save
ggsave("figs_temp/investment_costs_andopex650_SSA.png", plot = region_plot, width = 8, height = 13, dpi = 300)
```
