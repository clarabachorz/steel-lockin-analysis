...
df_production_global <- df_production %>%
  rename_all(clean_column_names) %>%
  group_by(Historic_Time) %>%
  summarise(
    Global_steel_production = sum(value))

df_production_regions <- df_production %>%
  rename_all(clean_column_names) %>%
  filter(Region %in% regions_to_filter)
...
df_plot_production <- df_production_regions %>%
  ...
  select(-IND, -OAS, -USA, -EUR, -REF, -NEU, -JPN) %>%
  pivot_longer(cols = c(CHA, USAEUR, SSA, INDOAS)) %>%
  mutate(Region = recode(name, !!!region_labels))

p <- ggplot(data=df_plot_production,
       aes(x=Historic_Time, y=value, colour=Region)) +
  geom_line()
