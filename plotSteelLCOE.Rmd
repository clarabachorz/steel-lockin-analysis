...
```
```{R}
plot_average_lcop <- function(lcop_avg, fs = 10, save_plot = FALSE){
  ###################################################
  ...
  # save plot
  if(save_plot){
    ggsave(filename = "./figs_temp/lcop_byroute_test.png", width = 6, height = 5, device='png', dpi=300)
  }
  return(p.drieaf)
}

p.drieaf <- plot_average_lcop(lcop_avg)
p.drieaf
...
plot_weighted_lcop <- function(lcop_weighted, fs=10, save_data = FALSE, save_plot = FALSE){
  ...
  if(save_data){
    write.csv(lcop_plot, file = "./inputdata/other/lcop_avg_test.csv", row.names = FALSE)
  }
  ...
  p.lcop <- ggplot() +
    ...
    labs(y = "LCOP [$/t]", fill="Cost Component",
        x = "Period", title = "Average levelized cost of steel, weighted by production route (Fast, 820)")
  if(save_plot){
    ggsave(filename = "figs_temp/lcop_avg_test.png", width= 8, height = 6, device='png', dpi=300)
  }
  return(p.lcop)
}
p.lcop <- plot_weighted_lcop(lcop_weighted)
p.lcop
