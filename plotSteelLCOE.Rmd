...
# df.priceFuel %>% 
  filter(entyFE == "fesos")  %>% 
  filter(period > 2025) %>%
# Using emifac + CO2 prices seems to give strange values
  # left_join(df.emiFac, by=c("region", "period", "entySE", "entyFE")) %>%
  # left_join(df.priceCO2, by = c("region", "period")) %>%
  #   mutate(emicost = ifelse(is.na(emiFac), 0, emiFac) * priceCO2) %>%
  # select(-emiFac, -priceCO2)
# instead simply use SE prices to get a rough idea
...
# print(lcop_plot %>% filter(value < 0) %>% arrange(region), n= Inf, width = Inf)

###save if needed
# write.csv(lcop_plot, file = "./inputdata/other/lcop_avg_npi.csv", row.names = FALSE)

## write csv without sec steel (need to uncomment above)
# write.csv(lcop_plot, file = "./inputdata/other/lcop_avg_npi_onlypri.csv", row.names = FALSE)
...
plotted_regions1 <- c("EUR", "CHA", "USA", "IND")

lcop_plot <- lcop_plot %>%
  filter(region %in% plotted_regions1, period %in% plotted_years1) %>% 
  # mutate(route_id = route %>% as.factor() %>% as.numeric()) %>% 
  # mutate(year_id = period %>% as.factor() %>% as.numeric()) %>% 
  # mutate(n_routes = max(route_id)) %>%
  # mutate(year_label_pos = (n_routes+2) * year_id) %>% 
