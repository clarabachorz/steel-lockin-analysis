...
```{r load, echo = FALSE}
#LOAD GDX
max_lockin <- "inputdata/gdx/GreenPolicy-PkBudg650.gdx"
...
lcop_2r <- lcop_plot %>%
  ...
  filter(route %in% c("DRI-EAF H2", "BF-BOF", "DRI-EAF NG")) %>%
...
lcop_2r <- lcop_2r %>%
  ...
  mutate(pos_offset = ifelse(route == "DRI-EAF H2", pos + 0.3, ifelse(route == "BF-BOF", pos - 0.1, ifelse(route == "DRI-EAF NG", pos + 0.1, pos - 0.1))))
...
#HERE: calculate average lcop for PRIMARY steel (ie weighted by route)
...
plotted_years1 <- c(2025, 2030, 2035, 2040, 2045, 2050, 2055)
...
p.lcop <- ggplot() +
  ...
  theme(text=element_text(size=fs),
        ...
        strip.text = element_text(size = 1.2 * fs)) +
  labs(y = "LCOP [$/t]", fill="Cost Component",
      x = "Period", title = "Average levelized cost of steel, weighted by production route (Green policy 650)")
