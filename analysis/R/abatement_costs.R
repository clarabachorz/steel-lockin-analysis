...
add_demand_adjustment <- function(scenario2_costs, demand_value) {
    scenario2_costs %>%
        ...
        mutate(total_cost = total_cost + demand_value) %>%
        select(-demand_value)
}
...
run_abatement_analysis <- function(scen1, scen2, region_to_aggr, start_date, end_date, discount_rate) {
    ...
    # Discounted sums
    costs1_disc <- calc_cumulated_discounted_sum(costs1, start_date, end_date, discount_rate, "total_cost", "cost1")
    costs2_disc <- calc_cumulated_discounted_sum(costs2_adj, start_date, end_date, discount_rate, "total_cost", "cost2")
    ...
    fscp_df <- costs1_disc %>%
        left_join(costs2_disc, by = "region") %>%
