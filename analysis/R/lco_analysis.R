...
calc_lcop <- function(gdx) {
    ###################################################
    # DEFINE KEY GENERIC SETS
    ...
    #### Take fuel price differentiated by SE, then take weighted sum according to SE shares.
    ...
    #### Prices obtained from SE sectors are not available before 2030 in some scenarios, so we take pm_FEPrice for these
    ...
    if (nrow(df.priceFuel %>% filter(period < 2030)) == 0) {
        message("No priceFuel data before 2030 found. Adding from pm_FEPrice.")
        # Add missing years from df.priceFuel_before_2030
        df.priceFuel <- bind_rows(
        df.priceFuel,
        df.priceFuel_before_2030 %>% 
            select(region, period, entyFE, priceFuel = priceFuel2)
        ) %>%
        arrange(region, period, entyFE)
    } else {
        message("priceFuel data before 2030 already present. No action taken.")
    }

    # Subtract FE CO2 tax from fuel price
    df.priceFuel <- df.priceFuel %>%
    ...
    df.CAPEX <- df.CAPEX %>% 
        left_join(df.discFac, by=c("all_te", "region")) %>% 
    ...
    ###################################################
    # JOIN PROCESSES AND CALCULATE LCOP
    ...
    lcop <- lcop %>% 
        full_join(tePrc2route, by=c("all_te","opmoPrc")) %>%
...
### ensure all time steps are covered in 5yr intervals, 
### even beyond 2060 when the model jumps by 10 year timesteps
add_5y_timesteps <- function(df, gdx) {
    ### load useful sets and parameters
    tePrc <- readGDX(gdx,"tePrc")
    tePrcAll <- tePrc
    tePrc2route <- as.data.frame(readGDX(gdx,name=c("tePrc2route"),format="first_found",restore_zeros = F)) %>%
        rename(all_te = tePrc)
...
rolling_avg_lcop <- function(lcop, gdx){
    ### load useful sets and parameters
    tePrc <- readGDX(gdx,"tePrc")
    tePrcAll <- tePrc
    tePrc2route <- as.data.frame(readGDX(gdx,name=c("tePrc2route"),format="first_found",restore_zeros = F)) %>%
    ...
    lcop_avg <- add_5y_timesteps(lcop_avg, gdx)

    #add lifetime info and end date
    lcop_avg <- lcop_avg %>%
        left_join(as.quitte(lifetime) %>%
            select(all_te, value, region) %>%
        rename(lifetime = value), by=c("all_te", "region")) %>% 
        mutate(end_period = period + lifetime) %>%
        #we have no info after 2100. The more interesting period is earlier anyways
        filter( period >= 2020) %>% 
        select(-lifetime) %>% 
        #calculate mean values over lifetime of the plant (until "end_period")
        rowwise() #%>%
...
########################
## TAKE WEIGHTED AVG VALUES
### add averaging routine: each time step has an average LCOP: each route is weighted by its production volume,
### df.outflowPrc. By default, this is for all types of steelmaking (primary + secondary production)

weighted_avg_lcop <- function(lcop,gdx){
    ...
    ccs_lookup <- df.outflowPrc %>%
        #h2 dri does not have ccs as an option, keep only ng
        filter(opmoPrc != "h2") %>%
        group_by(region, period, all_te) %>%
        summarize(value = sum(value)) %>%
...
plot_weighted_lcop <- function(gdx, fs=10, save_data = FALSE, save_plot = FALSE, scenario_name = ""){
    #calculate the LCOP
    lcop <- calc_lcop(gdx)
    lcop_weighted <- weighted_avg_lcop(lcop, gdx)
    ...
    ###save data if needed, for the abatement cost analysis
    if(save_data){
        fname <- tolower(paste0("./inputdata/other/lcop_avg_", gsub(" ", "_", scenario_name), ".csv"))
        write.csv(lcop_plot, file = fname, row.names = FALSE)
    }
    ...
    plotted_years <- c(2030, 2035, 2040, 2045, 2050, 2055, 2060)
    ...
    if(save_plot){
        ggsave(filename = paste0("./figs/lcop/lcop_avg_", gsub(" ", "_", scenario_name), ".jpg"),
            width= 8, height = 6, device='jpg', dpi=300)
    }
    return(p.lcop)
}

plot_average_lcop <- function(gdx, fs = 10, save_plot = FALSE, scenario_name = ""){
    lcop <- calc_lcop(gdx)
    lcop_avg <- rolling_avg_lcop(lcop, gdx)
  ###################################################
  ...
  ## uncomment if you want to plot average lcop value over plant lifetime
    ...
    # lcop_plot <- lcop %>%
        ...
        group_by(period, region, component, route) %>%
        summarize(value = sum(value), color=first(color), .groups = "drop") 
        # the model sometimes finds negative SE prices in later timesteps,
        # when fossil coal demand drops fast. We show this in the LCOP plots
        # %>%
        # mutate(value = pmax(value, 0.))

  ### add BF-BOF-CCS retrofit values
  ### (substract BF-BOF CAPEX as the plant is already build)
    capex_adjustment <- lcop_plot %>%
        ...
        mutate(value = value_ccs - value_bof,
            route = "BF-BOF CCS retrofit") %>%
        select(period, component = component_ccs, region, route, value, color = color_ccs)
  ...
  ## what if the base case is an already operating BF-BOF plant?
  ## In which case we remove the CAPEX of the BF-BOF, as this has already been payed
  ...
  # ### plot parameters
    plotted_years <- c(2035, 2040, 2050)
    ...
    year_label <- year_label %>% 
        mutate(pos = period %>% as.factor() %>% as.numeric())
    xmin <- min(lcop_plot$pos) - 0.7
    xmax <- max(lcop_plot$pos) + 0.7


    unique_component_names <- unique(as.character(component_names$component_name))

    lcop_plot$component <- factor(lcop_plot$component, levels=unique_component_names)
    ...
    p.drieaf <- ggplot() +
        ...
        xlim(xmin, xmax)  +
        ylim(-201.,2200.) +
        geom_text(data=year_label, aes(label = period), x = year_label$pos, y = -200., size = 2, parse = T) +
        geom_text(
        data = label_data,
        aes(
            x = pos_offset,
        ...
        labs(title = paste0("Average LCOP over the lifetime of a newly built plant in a given year,\nfor different production pathways (", scenario_name, ")"),
            y = "LCOP [$/t]",
            fill="Cost Component")

    # save plot
    if(save_plot){
        ggsave(filename = paste0("./figs/lcop/lcop_byroute_", gsub(" ", "_", scenario_name), ".jpg"),
             width = 6, height = 5, device='jpg', dpi=300)
    }
    return(p.drieaf)
}