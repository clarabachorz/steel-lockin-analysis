...
#### FORBID_NG_DRI is used to test the possibility to switch directly to H2-DRI.
#### In this case, all steel production from NG-DRI is instead assumed to come from H2-DRI.
calc_total_investments <- function(gdx, FORBID_NG_DRI = FALSE, SEPARATE_EL_DEMANDS = FALSE) {
  ...
  if (FORBID_NG_DRI) {
    # calculate NG DRI production
    ng_dri_outflow <- df.outflowPrc %>%
      filter(all_te == "idr", opmoPrc == "ng") %>%
      group_by(region, period) %>%
      summarise(ng_prod = sum(outflowPrc), .groups = "drop")
    
    # add ng-dri production to h2-dri so it is taken into account when calculating e.g. required elh2 capacity
    df.outflowPrc <- df.outflowPrc %>%
      left_join(ng_dri_outflow, by = c("region", "period")) %>%
      mutate(
        outflowPrc = ifelse(all_te == "idr" & opmoPrc == "h2", outflowPrc + ng_prod, outflowPrc),
        outflowPrc = ifelse(all_te == "idr" & opmoPrc == "ng", 0, outflowPrc)
      ) %>%
      select(-ng_prod)
  }
  ...
  #### we keep an option to separate out these electricity demands for the different technologies

if (SEPARATE_EL_DEMANDS) {

  ## This assumes that all DRI + pri-EAF have their own (combined) dedicated solar PV capacity.
  # Sec-EAF is separate (plants could be situated elsewhere).
    ...
    df.elDem <- df.specFeDemEl %>%
      ...
      mutate(
        pv_group = case_when(
          (all_te == "idr")| (all_te == "eaf" & opmoPrc == "PRI") ~ "DRI-EAF",
          TRUE ~ "EAF (secondary)"
        )
      ) %>%
      group_by(region, period, pv_group, entyFE) %>%
      summarise(totaldem = sum(dem))

      # Calculate required deltacap for each group
    df.deltacap_elDem <- df.elDem %>%
      group_by(pv_group) %>%
      group_modify(~calculate_required_deltacap(.x, df.capfac_el)) %>%
      ungroup() %>%
      mutate(component = paste0("Solar PV (",pv_group, ")")) %>%
      mutate(all_te = "spv")
  ...
  # left join with capex to calculate the total investments
...
calc_cash_flows_wacc_sensitivity <- function(gdx, FORBID_NG_DRI = FALSE) {
  ...
  #get total investments for the steel sector
  # we set SEPARATE_EL_DEMANDS = TRUE, as we need to obtain the specific PV investments for DRI-EAF only,
  # excluding secondary EAF.
  df.Inv_steel_all <- calc_total_investments(gdx, FORBID_NG_DRI, SEPARATE_EL_DEMANDS = TRUE)
  ...
  lifetime <- readGDX(gdx, name="pm_data", restore_zeros = F)[,,"lifetime"][,,][,,"lifetime"]
  ...
  df.total_Inv_long <- df.total_Inv %>%
    ...
    select(region, period, lifetime, wacc, discFac, annual_payment_period, end_lifetime, component)
  ...
  df_cashflows <- df_vintages %>%
    ...
    unnest(years) %>%
    rename(year = years) %>%
    group_by(region, component, wacc, discFac, year) %>%
    summarise(
...
combine_cash_flows <- function(scenarios, FORBID_NG_DRI = FALSE) {
  # scenarios: list of scenario objects, each with $gdx and $name
  # calc_fun: function to calculate cash flows, e.g. ccalc_cash_flows_wacc_sensitivity
  dfs <- lapply(scenarios, function(scen) {
    calc_cash_flows_wacc_sensitivity(scen$gdx, FORBID_NG_DRI) %>% mutate(scenario = scen$name)
  })
...
plot_cash_flows <- function(scenarios, region_to_plot){
  df_cashflows <- combine_cash_flows(scenarios)

  df_cashflows_noNG <- combine_cash_flows(scenarios, FORBID_NG_DRI = TRUE)


  df_plot <- df_cashflows %>%
    filter(region == region_to_plot, scenario == "Fast transition") %>%
      mutate(
        wacc_f = factor(
          wacc,
  ...
  cash_flow_plot <- ggplot(df_plot_combined, aes(x = year, y = annual_cashflow, fill = component)) +
    geom_area(color = "grey", linewidth = 0.2) +
    facet_grid(wacc_f ~ case) +
    scale_fill_manual(values = component_colors) +
    scale_x_continuous(
