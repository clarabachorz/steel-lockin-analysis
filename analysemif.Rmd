...
```{r load data}
mifdata1 <- read.quitte("inputdata/mif/REMIND_generic_PolicyExtended-PkBudg650.mif")
...
base_list <- list(mifdata7, mifdata8, mifdata9, mifdata10)
h2_list <- list(mifdata1, mifdata2, mifdata3)
...
# Get unique scenario names
unique_scenarios <- unique(mainscens_emi_df_plot$scenario)
unique_scenarios <- c("Base scenario (1.5C)", 
                      "Green Policy (1.5C)",
                      "Base scenario (WB2C)",
                      "Green Policy (WB2C)")
...
# find x-coordinates of bars
x_base <- which(levels(mainscens_emi_df_plot$scenario) == "Base scenario (1.5C)")
x_others <- which(levels(mainscens_emi_df_plot$scenario) != "Base scenario (1.5C)")


y1 <- mainscens_emi_df_plot$cumu_CO2[mainscens_emi_df_plot$scenario == "Base scenario (1.5C)"] / 1e3
...
# find y-coordinates for arrows
y_base <- y1 + 10  # slightly above base bar
y_others <- mainscens_emi_df_plot$cumu_CO2[mainscens_emi_df_plot$scenario != "Base scenario (1.5C)"] / 1e3 + 30



# Plot
ggplot(mainscens_emi_df_plot, aes(x = scenario, y = cumu_CO2 / 1e3, fill = scenario)) +
  geom_bar(stat = "identity", width = 0.6) +
  scale_x_discrete(labels = function(x) str_wrap(x, width = 20)) +
  # scale_fill_manual(values = c("#d7a614", "#3fa60e", "#bb5700", "#1a7213")) +
  scale_fill_manual(values = c("#88CCEE", "#0072B2", "#FDB863", "#D55E00")) +
  labs(
    title = "Cumulative CO2 Emissions (2025-2070)\nfrom steel production, by scenario",
    x = "Scenario",
    y = "Cumulative CO2 (Gt CO2)"
...
#save
ggsave("figs_temp/cumu_emi_base_scen.png", width = 8, height = 5, dpi = 300)
