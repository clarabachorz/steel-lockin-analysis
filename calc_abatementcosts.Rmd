...
p <- ggplot(df, aes(xmin = xmin, xmax = xmax, ymin = 0, ymax = fscp, fill = region)) +
  ...
  labs(
    ...
    title = "Marginal abatement cost: from NPi to Fast transition",
  ...
  theme_minimal() +
    ...
    annotate("text", x = 30, y = price_carbon$co2_price + 20, 
             ...
             color = "black")
p
# ggsave("figs_temp/abatement_costs_steel_NPitoFT.png", p, width = 8, height = 6)
```
```{R plot}

ggplot(total_costs, aes(x = period, y = fscp)) +
    geom_line() +
    facet_wrap(~ region, scales = "free_y") +
    labs(title = "Abatement costs of steel production",
         x = "Year",
         y = "Abatement cost (USD/tCO2)") +
    theme_minimal() +
