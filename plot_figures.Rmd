---
title: "Notebook: replicating article figures for \"Averting the steel carbon lock-in through strategic green investments\""
author: "Clara Bachorz"
output: html_document
---


In this notebook, we replicate the figures and analyses shown in the paper.

## Setup

Make sure to run all the cells in this section.
...
required_packages <- c( "magclass", "waterfalls", "dplyr", "gridExtra", "grid", "purrr", "ggpubr", "ggplot2", "quitte", "tidyverse", "mrremind", "geomtextpath")
...
source("./analysis/R/steel_production_plots.R")
...
scenario_TWLI_40yrBFBOF <- list(
  name = "Transition with Lock-in 40yr BF-BOF",
  mif_file = "inputdata/mif/REMIND_generic_TransitionwLockIn_40yrBFBOF-PkBudg820.mif"
)

scenario_FT_40yrBFBOF <- list(
  name = "Fast Transition 40yr BF-BOF",
  mif_file = "inputdata/mif/REMIND_generic_FastTransition_40yrBFBOF-PkBudg820.mif"
)
...
## Preparatory calculations: GEM GIST data and levelized cost of steel from REMIND scenario data

These output of these calculations are already in the repository, under inputdata/gem and inputdata/other.
If you wish to reproduce the calculation, run the cells below. Otherwise, feel free to skip ahead to the next section
to plot the figures.
...
### Calculate average levelized cost of steel (LCOP) from REMIND scenarios
...
```{r calc and plot LCO, eval = FALSE}
...
```{r plot plant level data, fig.width=18/2.54, fig.height=12/2.54}
...
## Plot Figure 3
The chunk below includes different steel production plots (by route) from REMIND scenario results.

```{r plot steel production data, fig.height = 18/2.54, fig.width = 18/2.54}
# Plot Figure 3 and Figure ED2
plot_stacked_production_plots(mif_list)

# Plot Figure ED1 (partial reproduction without annotations, these were added manually)
plot_area_production_plots(mif_list)

```

## Plot Figure 4
This figure is composed of three different elements: 
1. Average abatement costs for the emission reductions achieved between each of the three scenarios, combined with 
2. an emission cascade plot (Figure 4a), as well as
3. a steel system cost vs cost of other mitigation option plot (Figure 4b).

### 1. Get average abatement costs
...
run_and_save_abatement_cost_plot <- function(scenario1, scenario2, region_to_aggr, start_date, end_date, discount_rate, save_plot = FALSE, filename = NULL) {
  ...
  #save
  if (save_plot) {
    ggsave(paste0(filename, ".jpg"), plot = results$plot, width = 180, height = 90, unit = "mm")
    ggsave(paste0(filename, ".pdf"), plot = results$plot, width = 180, height = 90, unit = "mm")
    }
...
df_aac_combined <- rbind(df_plot1, df_plot2) %>%
  select(fscp, region, category, scenario1, scenario2) %>%
  group_by(region, scenario1, scenario2) %>%
  summarise(fscp = sum(fscp)) %>%
  ungroup()
```

### Add 2. and 3. to plot Figure 4:
...
We first plot Figure 4b., which shows the total additional cost of the *Fast Transition* scenario compared to
*Transition with Lock-in* scenario (over a defined period and discount rate). 
This is compared to the value of the additional CO2 abated (calculated using CO2 price * emission reduction between the two scenarios). 

Then, we combine this plot (`p_scenariocost_vs_co2value`) with the emission cascade using `combine_cascade_cumueum_and_co2value_plot`. 

```{r plot cumulative emissions cascade, fig.height = 18/2.54, fig.width = 18/2.54}

# Make Fig 4b: scenario additional costs vs co2 value
...
## Combine all elements in Figure 4:
...
```
## Plot Figure 5: Investments required in India in the different scenarios

We also plot Figure ED4 in this code block: the remaining expenditure required from other fuels (fossil gas, biogas, fossil coal, biomass solids).
```{r calc investment costs, fig.height = 9/2.54, fig.width = 18/2.54}
...
# plot the cash flows required to pay back DRI-EAF investments (from Figure 5)
# does not include cash flows required for secondary EAF investments
plot_cash_flows(scenarios, region_to_plot = "IND")
...
## Other Supp Figs: energy prices for the main scenarios
...
# Supp Fig: which sector abates additional emissions in TwLI relative to FT
plot_em_diff_by_sector(mif_list, fig_name = "FigureED4")


# Supp Fig: steel production scenarios, with additional sensitivity of 40yr average lifetime for BF-BOFs (instead of 35yr)
scens_40yr <- list(scenario_TWLI, scenario_TWLI_40yrBFBOF, scenario_FT, scenario_FT_40yrBFBOF)
mif_list_40yr <- lapply(scens_40yr, function(scen) {
  mif <- read.quitte(scen$mif_file)
  mif
})

plot_REMIND_steel_production(mif_list_40yr, fig_name = "FigureS9")


## Supp. Fig
# get CO2 emissions from the REMIND scenarios
cumu_em <- calc_global_cumuem(mif_list)
#combine with GEM data (plant level estimation of lock-in)
plot_simplified_cumu_emissions(cumu_em)
```
