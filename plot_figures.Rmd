...
required_packages <- c( "magclass", "waterfalls", "dplyr", "gridExtra", "grid", "purrr", "ggpubr", "ggplot2", "quitte", "tidyverse", "mrremind")
...
df_plot <- df_cashflows %>%
  filter(region == "IND", scenario == "Fast transition") %>%
    mutate(wacc_f = factor(
      wacc,
      levels = sort(unique(wacc)),
      labels = scales::percent(sort(unique(wacc)), accuracy = 1)
    ))

ggplot(df_plot, aes(x = year, y = annual_cashflow, color = wacc_f)) +
  ...
  labs(
    title = "India: Required annual capital cash flow for DRI-EAF investments",
    x = "",
    y = "Required annual cash flow [billion USD/year]",
    color = "WACC"
  ) +
  theme_minimal()
ggsave("India_cashflow_plot.png", plot = last_plot(), dpi = 300)
```
...
# plot_em_diff_by_sector(mif_list, fig_name = "FigureS9")
...
plot_REMIND_steel_production(mif_list_40yr, fig_name = "FigureS10")
